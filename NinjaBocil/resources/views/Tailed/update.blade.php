...
<body>
    <div class="p-4 sm:ml-64 sm:mt-23  min-h-screen dark:bg-gray-800">
        ...
        <form method="POST" action="/tailed/updatedata/{{ $tailed->id }}" enctype="multipart/form-data"
            class="create_tailed">
            @csrf
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    ...
                    <input
                        ...
                        onchange="previewImage()" value="{{ Storage::url('public/image/' . $tailed->foto_tailed) }}">
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_input_help">SVG, PNG, JPG or GIF
                        (MAX.
                        800x400px).</p>
                    <br>
                    <img src="{{ Storage::url('public/image/' . $tailed->foto_tailed) }}" alt=""
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" id="nama_tailed" name="nama_tailed"
                        ...
                        placeholder="Nama" value="{{ $tailed->nama_tailed }}" required>
                </div>
                <div>
                    ...
                    <input type="text" id="slug_tailed" name="slug_tailed"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Slug" value="{{ $tailed->slug_tailed }}" disabled>
                </div>
            </div>
    ...
    <script>
        ...
        const skill1Select = document.getElementById('skill_1_tailed');
        const skill2Select = document.getElementById('skill_2_tailed');
        const skill3Select = document.getElementById('skill_3_tailed');
        const skill4Select = document.getElementById('skill_4_tailed');

        var skill1Tailed = '{{ $tailed->skill_1_tailed }}';
        var skill2Tailed = '{{ $tailed->skill_2_tailed }}';
        var skill3Tailed = '{{ $tailed->skill_3_tailed }}';
        var skill4Tailed = '{{ $tailed->skill_4_tailed }}';

        skill1Select.value = skill1Tailed;
        skill2Select.value = skill2Tailed;
        skill3Select.value = skill3Tailed;
        skill4Select.value = skill4Tailed;
    </script>
</body>
