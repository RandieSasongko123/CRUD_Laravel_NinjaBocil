...
<head>
    ...
    <link rel="shortcut icon" href="{{ asset('image/swordsmans.png') }}" type="x-icon">
</head>
...
<body class="{{ session('darkMode', false) ? 'dark' : '' }}">
                ...
                <tbody>
                    @foreach ($data as $d)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="px-6 py-4">
                                <img src="{{ Storage::url('image/' . $d->foto_karakter) }}" alt=""
                                    id="preview" style="width: 75px; height:75px;">
                            </td>
                            <td class="px-6 py-4">
                                {{ $d->grade_karakter }}
                            </td>
                            ...
                            <td class="py-4">
                                <a href="/karakter/update/{{ $d->id }}"
                                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                <a href="/karakter/delete/{{ $d->id }}"
                                    class="font-medium text-red-600 dark:text-red-500 hover:underline ml-3">Delete</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <script type="text/javascript">
        document.getElementById('tambah').onclick = function() {
            window.location.href = "{{ route('Karakter.create') }}";
        };
    </script>
